...
void sem_wait(sem_t *sem)
{
   ...
   if (sem->contadorSem < 0) {
      ...
      do { if (f_aptos.ready_queue[f_aptos.task_running].task_stack.size > 0) { index = 0; f_aptos.ready_queue[f_aptos.task_running].task_state = WAITING_SEM; f_aptos.ready_queue[f_aptos.task_running].w_reg = WREG; f_aptos.ready_queue[f_aptos.task_running].bsr_reg = BSR; f_aptos.ready_queue[f_aptos.task_running].status_reg = STATUS; f_aptos.ready_queue[f_aptos.task_running].task_stack.size = 0; do { f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index] = TOS; index++; __asm("POP"); } while (STKPTR); } else { f_aptos.ready_queue[f_aptos.task_running].task_state = WAITING_SEM; } } while (0);;
...
void sem_post(sem_t *sem)
{
   ...
   if (sem->contadorSem <= 0) {
      ...
      sem->fila.next_to_free = (sem->fila.next_to_free + 1) % 4;
      sem->fila.queue_size--;
   }
