...
u_int scheduler(void);
u_int rr_scheduler(void);
u_int prior_scheduler(void);
# 5 "./dispatcher.h" 2
...
void task_delay(u_int ms){
 INTCONbits.GIE = 0;
 { index = 0; f_aptos.ready_queue[f_aptos.task_running].task_state = WAITING_DELAY; f_aptos.ready_queue[f_aptos.task_running].w_reg = WREG; f_aptos.ready_queue[f_aptos.task_running].bsr_reg = BSR; f_aptos.ready_queue[f_aptos.task_running].status_reg = STATUS; while (STKPTR) { f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSU_reg = TOSU; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSL_reg = TOSL; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSH_reg = TOSH; index++; __asm("POP"); } f_aptos.ready_queue[f_aptos.task_running].task_stack.size = index; }

 f_aptos.ready_queue[f_aptos.task_running].delay = ms;

 { f_aptos.task_running = scheduler(); STKPTR = 0; if (f_aptos.ready_queue[f_aptos.task_running].task_stack.size == 0) { __asm("PUSH"); TOS = f_aptos.ready_queue[f_aptos.task_running].task_func; f_aptos.ready_queue[f_aptos.task_running].task_state = RUNNING; } else { f_aptos.ready_queue[f_aptos.task_running].task_state = RUNNING; WREG = f_aptos.ready_queue[f_aptos.task_running].w_reg; BSR = f_aptos.ready_queue[f_aptos.task_running].bsr_reg; STATUS = f_aptos.ready_queue[f_aptos.task_running].status_reg; index = f_aptos.ready_queue[f_aptos.task_running].task_stack.size; while(index) { __asm("PUSH"); TOSU = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSU_reg; TOSL = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSL_reg; TOSH = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSH_reg; index--; } } };
 INTCONbits.GIE = 1;
}
...
void task_yield(){
 INTCONbits.GIE = 0;
 { index = 0; f_aptos.ready_queue[f_aptos.task_running].task_state = READY; f_aptos.ready_queue[f_aptos.task_running].w_reg = WREG; f_aptos.ready_queue[f_aptos.task_running].bsr_reg = BSR; f_aptos.ready_queue[f_aptos.task_running].status_reg = STATUS; while (STKPTR) { f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSU_reg = TOSU; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSL_reg = TOSL; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSH_reg = TOSH; index++; __asm("POP"); } f_aptos.ready_queue[f_aptos.task_running].task_stack.size = index; };
 { f_aptos.task_running = scheduler(); STKPTR = 0; if (f_aptos.ready_queue[f_aptos.task_running].task_stack.size == 0) { __asm("PUSH"); TOS = f_aptos.ready_queue[f_aptos.task_running].task_func; f_aptos.ready_queue[f_aptos.task_running].task_state = RUNNING; } else { f_aptos.ready_queue[f_aptos.task_running].task_state = RUNNING; WREG = f_aptos.ready_queue[f_aptos.task_running].w_reg; BSR = f_aptos.ready_queue[f_aptos.task_running].bsr_reg; STATUS = f_aptos.ready_queue[f_aptos.task_running].status_reg; index = f_aptos.ready_queue[f_aptos.task_running].task_stack.size; while(index) { __asm("PUSH"); TOSU = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSU_reg; TOSL = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSL_reg; TOSH = f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index - 1].TOSH_reg; index--; } } };
 INTCONbits.GIE = 1;
}
