...
void sem_wait(sem_t *sem){
 ...
 if (sem->contadorSem <= 0) {
  ...
  sem->fila.queue = (sem->fila.queue + 1) % (6 - 1);

  { index = 0; f_aptos.ready_queue[f_aptos.task_running].task_state = WAITING_SEM; f_aptos.ready_queue[f_aptos.task_running].w_reg = WREG; f_aptos.ready_queue[f_aptos.task_running].bsr_reg = BSR; f_aptos.ready_queue[f_aptos.task_running].status_reg = STATUS; while (STKPTR) { f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSU_reg = TOSU; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSL_reg = TOSL; f_aptos.ready_queue[f_aptos.task_running].task_stack.stack[index].TOSH_reg = TOSH; index++; __asm("POP"); } f_aptos.ready_queue[f_aptos.task_running].task_stack.size = index; };
...
void sem_post(sem_t *sem){
 ...
 if (sem->contadorSem < 0) {
  ...
  sem->fila.next_to_free = (sem->fila.next_to_free + 1) % (6 - 1);
 }
